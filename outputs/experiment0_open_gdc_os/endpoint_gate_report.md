# Endpoint gate report — open_gdc_os

endpoint_type: `overall_survival` · role: `technical_validation`

- technical_validation_claim_allowed: **YES**
- primary_biological_claim_allowed: **NO**
- relapse_or_pfs_claim_allowed: **NO**
- omics_increment: **SUGGESTIVE_NOT_CONFIRMED** (ΔC=+0.069, CI low=-0.033)
- evidence_level: **technical_validation_only**

This run uses an **overall-survival** endpoint and therefore CANNOT license a relapse/PFS or primary biological claim (technical-validation + residual-risk pilot, Experiment 0).
